...
// May throw
Data addressStringToData2(const std::string& asString) {
    if (asString.empty()) {
        return {};
    }
...
Transaction Signer::build(const Proto::SigningInput &input) {
    Data toAddress = addressStringToData2(input.to_address());
    ...
    {
        Data tokenToAddress = addressStringToData2(input.transaction().erc20_transfer().to());
    ...
    {
        Data spenderAddress = addressStringToData2(input.transaction().erc20_approve().spender());
    ...
    {
        Data tokenToAddress = addressStringToData2(input.transaction().erc721_transfer().to());
        Data tokenFromAddress = addressStringToData2(input.transaction().erc721_transfer().from());
    ...
    {
        Data tokenToAddress = addressStringToData2(input.transaction().erc1155_transfer().to());
        Data tokenFromAddress = addressStringToData2(input.transaction().erc1155_transfer().from());
